...
- **🖥️ CLI vs GUI 계정 분리 관리**:
  - `CLI (SSH/Telnet/Console/RDP)`: 원격 접속 및 콘솔 계정 관리 (단일 고유 `root` 관리자 역할 관리).
  - `GUI (웹콘솔)`: 관리 웹 UI 및 웹페이지 계정 관리 (역할 개념 없이 자유로운 ID/PW 관리).
- **👑 CLI root(관리자) 단일 고유화**:
  - 한 자산의 CLI 접속에서 `root` 역할은 오직 1개만 허용 (윈도우 `administrator`도 역할은 `root`로 통일).
  - CLI에 이미 관리자(root)가 존재하면 추가 CLI 계정은 자동으로 'user' 역할로 배정됨.
- **⚡ 전용 root 수정 모드**:
  - 관리자(root) 계정 수정 시 불필요한 ID/역할 질문 없이 **패스워드 및 메모만 즉시 수정**.
- **🗑️ 세분화된 자산/계정 삭제**:
  - 계정이 2개 이상 등록된 자산 삭제 시 **[1] 특정 계정만 골라서 삭제** 또는 **[2] 자산 전체 일괄 삭제** 선택 가능.
...
- **🎮 방향키(키패드) 인터랙티브 메뉴 & 단축키 지원**:
  - 키보드 화살표(`↑`, `↓`, `←`, `→`)로 화살표 커서(`▶`)를 움직여 `Enter`로 선택 가능 (숫자 단축키도 동시 지원).
- **🔍 통합 스마트 검색**:
